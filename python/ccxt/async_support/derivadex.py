...
class derivadex(Exchange):
    ...
    async def fetch_markets(self, params={}):
        ...
        """
        params['kind'] = 0
        response = await self.publicGetSpecs(params)
        # {
        #     "value": [
        #         {
        #             "kind": 0,
    ...
    async def fetch_ticker(self, symbol, params={}):
        ...
        tickers = await self.fetch_tickers([market['id']], params)
    ...
    async def fetch_tickers(self, symbols=None, params={}):
        ...
        for i in range(0, len(symbols)):
            ticker = await self.construct_ticker(symbols[i])
            if ticker is not None:
                result[symbols[i]] = ticker
        return result

    async def construct_ticker(self, symbol):
        params = {}
        params['symbol'] = symbol
        params['depth'] = 1
        orderBookResponse, tickerResponse = await asyncio.gather(*[
    ...
    async def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        ...
        response = await self.publicGetFills(extendedRequest)
    ...
    async def fetch_trades(self, symbol, since=None, limit=None, params={}):
        ...
        if since is not None:
            request['since'] = since / 1000
        request['order'] = params['order'] is not params['order'] if None else 'asc'
        response = await self.publicGetFills(self.extend(request, params))
        # {
        #     value: [
        #       {
        #         epochId: '27',
        ...
        #     ]
        # }
        return await self.parse_trades_custom(response, market, since, limit)

    async def get_order_intents(self, trades):
        result = {}
        params = {
            'orderHash': [],
        }
        ...
        orderIntentResponse = await self.publicGetOrderIntents(params)
    ...
    async def parse_trades_custom(self, trades, market=None, since=None, limit=None, trader=None, strategy=None):
        trades = self.to_array(trades)
        result = []
        orderIntents = await self.get_order_intents(trades[0])
    ...
    async def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=None, params={}):
        ...
        if since is not None:
            request['since'] = since
        response = await self.v2GetRestOhlcv(self.extend(request, params))
    ...
    async def get_sequenced_order(self, operatorResponse, lookbackLimit=None):
        """
        ...
        response = await self.publicGetAccountTraderStrategyStrategyIdOrderIntents(params)
        order = response['value'].find((intent) => intent.nonce == operatorResponse.c.nonce)
        if order is None:
            await self.wait(1000)  # retry after 1 second
            response = await self.publicGetAccountTraderStrategyStrategyIdOrderIntents(params)
            order = response['value'].find((intent) => intent.nonce == operatorResponse.c.nonce)
            if order is None:
                raise OrderNotFound(self.id + ' getSequencedOrder() could not find the order intent associated with self operator response')
        return await self.parse_order(order)

    async def fetch_public_orders(self, symbol=None, since=None, limit=None, params={}):
        """
        ...
        if since is not None:
            request['since'] = since / 1000
        request['order'] = params['order'] is not params['order'] if None else 'asc'
        response = await self.publicGetOrderIntents(request)
    ...
    async def fetch_order(self, id, symbol=None, params={}):
        ...
        response = await self.publicGetOrderIntents(request)
        return await self.parse_order(response['value'][0], market)
    ...
    async def fetch_orders(self, symbol=None, since=None, limit=None, params={}):
        ...
        if since is not None:
            request['since'] = since / 1000
        request['order'] = params['order'] is not params['order'] if None else 'asc'
        response = await self.publicGetAccountTraderStrategyStrategyIdOrderIntents(request)
        return await self.parse_orders_custom(response['value'], market, since, limit)

    async def parse_orders_custom(self, orders, market=None, since=None, limit=None, params={}):
        results = []
    ...
    async def parse_order(self, order, market=None):
        ...
        fillResponse = await self.publicGetFills(params)
    ...
    async def update_profile(self, payFeesInDDX):
        """
        ...
        operatorResponse = await self.get_operator_response_for_order_intent(orderIntent, 'ProfileUpdate')
    ...
    async def cancel_all_orders(self, symbol=None, params={}):
        ...
        operatorResponse = await self.get_operator_response_for_order_intent(orderIntent, 'CancelAll')
        if operatorResponse['t'] != 'Sequenced':
    ...
    async def cancel_order(self, id, symbol=None, params={}):
        ...
        operatorResponse = await self.get_operator_response_for_order_intent(orderIntent, 'CancelOrder')
        timestamp = Date.now()
        if operatorResponse['t'] != 'Sequenced':
    ...
    async def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        operatorResponse = await self.get_operator_response_for_order_intent(orderIntent, 'Order')
        if operatorResponse['t'] != 'Sequenced':
        ...
        if not params.getOrderConfirmation:
            return operatorResponse
        return await self.get_sequenced_order(operatorResponse, 10)

    async def get_operator_response_for_order_intent(self, orderIntent, requestType):
        scaledOrderIntent = requestType == self.get_scaled_order_intent(orderIntent) if 'Order' else orderIntent
        encryptionKey = self.encryptionKey
        if encryptionKey is None:
            encryptionKey = await self.v2GetEncryptionKey()
        ...
        encryptedIntent = await self.encrypt_intent(encryptionKey, intent)
        buffer = Buffer.from(encryptedIntent.replace(/^0x/, ''), 'hex')
        return await self.v2PostRequest(buffer)
    ...
    async def encrypt_intent(self, encryptionKey, payload):
    ...
    async def fetch_balance(self, params={}):
        ...
        strategyResponse = await self.publicGetAccountTraderStrategyStrategyId(strategyRequest)
        # {
        #     value: {
    ...
    async def fetch_deposit_address(self, code, params={}):
        ...
        """
        addresses = await self.rawGetSnapshotAddresses({'contractDeployment': 'beta'})  # TODO: switch to mainnet
    ...
    async def fetch_positions(self, symbols=None, params={}):
        ...
        """
        response = await self.publicGetPositions()
    ...
    async def fetch_funding_rate(self, symbol, params={}):
        ...
        """
        response = await self.publicGetMarkets({'symbol': symbol})
        response['value'].forEach((rate) => {
            rate.timestamp = response['timestamp'] * 1000
        })
